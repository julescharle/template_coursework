...
Below are my examples to add on to the department template.

I will be using the Palmer Penguins R package [@palmerpenguins] for my data
...
#| fig-cap: "Plot"
data("penguins")
f <- ggplot(penguins, aes(x=species, y= body_mass_g, colour = species, fill = species)) + geom_boxplot() + labs(x = "Species", y = "Bodymass", title = "Average Body Mass By Species") + theme_linedraw()
...
We can also generate some tables 

```{r, echo = F}
kable(head(penguins))
```

And we can reference figures, for example @fig-plot
